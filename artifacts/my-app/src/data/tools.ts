...
function T(
  skillId: SkillId, tier: number, name: string,
  levelRequired: number, speedBonus: number, durability: number,
): Tool {
  return { 
    id: `${skillId}_tool_t${tier}`, 
    skillId, 
    icon: getToolIcon(skillId),
    tier, 
    name, 
    levelRequired, 
    speedBonus, 
...
export const TOOLS: Tool[] = [
  ...
  // Mining — кирки
  T('mining', 1, 'Медная кирка',     1,  0,  100),
  T('mining', 2, 'Бронзовая кирка',   15, 5,  150),
  T('mining', 3, 'Железная кирка',   30, 10, 200),
  T('mining', 4, 'Стальная кирка',   50, 20, 300),
  T('mining', 5, 'Мифриловая кирка', 75, 30, 500),
