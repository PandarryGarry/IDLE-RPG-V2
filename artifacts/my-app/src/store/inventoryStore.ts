...
export const useInventoryStore = create<InventoryStore>((set, get) => ({
  ...
  // Сумма по ВСЕМ слотам с этим ключом (экипировка теперь в нескольких слотах)
  getItemQty: (itemId, tier, grade) =>
    get().items
      .filter(s => s.itemId === itemId && s.tier === tier && s.grade === grade)
  ...
  addItem: (itemId, qty, tier, grade) => {
    ...
    const free = maxSlots - usedSlots;

    // ── ЭКИПИРОВКА: каждый экземпляр = отдельный слот ──
    if (isEquipment) {
      const addCount = Math.min(qty, free);
      if (addCount <= 0) return false;
      const newSlots: ItemSlot[] = Array.from({ length: addCount }, () => ({
        itemId, quantity: 1, locked: false, tab: 0, tier, grade,
      }));
      set({ items: [...items, ...newSlots] });
      return addCount === qty;
    }

    // ── Остальное: стак по ключу itemId+tier+grade ──
    const existingIdx = items.findIndex(s => s.itemId === itemId && s.tier === tier && s.grade === grade);
  ...
  // Снимает количество ЧЕРЕЗ несколько слотов с тем же ключом
  removeItem: (itemId, qty, tier, grade) => {
    if (get().getItemQty(itemId, tier, grade) < qty) return false;
    ...
    for (let i = newItems.length - 1; i >= 0 && remaining > 0; i--) {
      const s = newItems[i];
      if (s.itemId === itemId && s.tier === tier && s.grade === grade) {
        const take = Math.min(s.quantity, remaining);
        const newQty = s.quantity - take;
        remaining -= take;
        if (newQty <= 0) newItems.splice(i, 1);
        else newItems[i] = { ...s, quantity: newQty };
      }
    }
  ...
  // Миграция: разделяем старые стаки экипировки, если хватает места
  loadFromSave: (items, gp, maxSlots) => {
    const normalized: ItemSlot[] = [];
    for (const slot of items) {
      const item = getItem(slot.itemId);
      if (item?.equipSlot && slot.quantity > 1) {
        for (let i = 0; i < slot.quantity; i++) {
          normalized.push({ ...slot, quantity: 1 });
        }
      } else {
        normalized.push(slot);
      }
    }
    const fits = normalized.filter(s => s.quantity > 0).length <= maxSlots;
    set({ items: fits ? normalized : items, gp, maxSlots });
  },
